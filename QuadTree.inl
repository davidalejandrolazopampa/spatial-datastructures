...
std::vector<Point> QuadTree<Node, Rectangle, Point>::range_new_qd(Rectangle region, std::shared_ptr<Node>& node) {
    ...
    if (!(node == nullptr)) {
        ...
        if (!(oeste_limite.get(0) <= P_temporal.get(0)) || !(P_temporal.get(0) <= noreste_limite.get(0)) ||
            !(oeste_limite.get(1) <= P_temporal.get(1)) || !(P_temporal.get(1) <= noreste_limite.get(1))) {}
        else Resultado.push_back(P_temporal);

        //Verificacion, llamada recursiva
        std::vector<Point> point_NW = range_new_qd(region, node->NW());
        if (!(point_NW.size() >= 1)) {} else Resultado.insert(Resultado.end(), point_NW.begin(), point_NW.end());

        std::vector<Point> point_NE = range_new_qd(region, node->NE());
        if (!(point_NE.size() >= 1)) {} else Resultado.insert(Resultado.end(), point_NE.begin(), point_NE.end());

        std::vector<Point> point_SE = range_new_qd(region, node->SE());
        if (!(point_SE.size() >= 1)) {} else Resultado.insert(Resultado.end(), point_SE.begin(), point_SE.end());

        std::vector<Point> point_SW = range_new_qd(region, node->SW());
